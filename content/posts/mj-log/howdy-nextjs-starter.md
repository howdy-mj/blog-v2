...
이래서 다들 CSS variable을 사용하는구나 깨달았다. 이럴 줄 알았다면 처음에 CSS 모듈을 생각했을 텐데.. 역시 해보지 않으면 모르는 구나를 다시 한번 깨달았다.

그리고 다크모드 구현이 끝난 줄 알았지만, 댓글 컴포넌트인 utterances의 테마가 같이 변경되지 않았다. 여기저기 레퍼런스를 찾아봤지만 원하던 결과가 나오지 않아, 결국 테마를 토글할 때 utterances의 로직을 넣어주었다. (utterances는 Github Repo가 public이어야 사용 가능해서 당분간은 사용 안할 예정이다)
